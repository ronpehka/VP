...
function readperson($selectedperson){ 
$notice = "<p>Kahjuks tegelasi ei leitud!</p> \n";
	...
	$names = "";
	while($stmt->fetch()){
		$names .= '<option value="' .$idfromdb .'"';
		if(intval($idfromdb) == $selectedperson){
			$names .=" selected";
		}
		$names .= ">" .$firstnamefromdb ." " .$lastnamefromdb." </option> \n";
	}
	if(!empty($names)){
		$notice = '<select name="nameinput">' ."\n";
		$notice .= '<option value="" selected disabled>Vali nimi</option>' ."\n";
		$notice .= $names;
		$notice .= "</select> \n";
	}
	$stmt->close();
...
function storenewperson_in_movierelation($selectedfilm, $selectedperson){
	$notice = "";
	$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("SELECT from person_in_movie WHERE person_id = ? AND movie_id = ?");
	echo $conn->error;
	...
	if($stmt->fetch()){
		...
		$stmt->close();
		$stmt = $conn->prepare("INSERT INTO person_in_movie (person.person_id and movie_id) VALUES(?,?)");
		echo $conn->error;
		$stmt->bind_param("ii", $selectedfilm, $selectedstudio);
		if($stmt->execute()){
			$notice = "Uus seos edukalt salvestatud!";
...
function readquotes($selectedquote){
	...
	while($stmt->fetch()){
		...
		if(intval($idfromdb) == $selectedquote){
			$quotes .=" selected";
		}
